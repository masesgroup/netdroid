...
package org.mases.netdroid.generated.android.database.sqlite;

public final class SQLiteTransactionListener implements org.mases.jcobridge.IJCListener, android.database.sqlite.SQLiteTransactionListener {
     ...
     */
    public SQLiteTransactionListener(String key) throws org.mases.jcobridge.JCNativeException {
        super();
    ...
    public void onBegin() {
        ...
        if (_onBeginIndex <= 0) _onBeginIndex = getEventIndex("onBegin");
        raiseEvent(_onBeginIndex, eventDataExchange); if (!eventDataExchange.getHasOverride()) throw new UnsupportedOperationException("The method shall be implemented in .NET side since does not have a default implementation within the JVM");
    }
    int _onCommitIndex = 0;
    //@Override
    public void onCommit() {
        org.mases.jnet.developed.JNetEventResult eventDataExchange = new org.mases.jnet.developed.JNetEventResult();
        if (_onCommitIndex <= 0) _onCommitIndex = getEventIndex("onCommit");
        raiseEvent(_onCommitIndex, eventDataExchange); if (!eventDataExchange.getHasOverride()) throw new UnsupportedOperationException("The method shall be implemented in .NET side since does not have a default implementation within the JVM");
    }
    int _onRollbackIndex = 0;
    //@Override
    public void onRollback() {
        org.mases.jnet.developed.JNetEventResult eventDataExchange = new org.mases.jnet.developed.JNetEventResult();
        if (_onRollbackIndex <= 0) _onRollbackIndex = getEventIndex("onRollback");
        raiseEvent(_onRollbackIndex, eventDataExchange); if (!eventDataExchange.getHasOverride()) throw new UnsupportedOperationException("The method shall be implemented in .NET side since does not have a default implementation within the JVM");
    }
